...
*/
pub struct CohortManager<A, B, C, D, F, G, O, H> {
    cohorts: HashMap<String, Vec<HealthDataContract<A, B, C, D, F, G, O, H>>>,  // Map from cohort_id to contracts
}

impl<A, B, C, D, F, G, O, H> CohortManager<A, B, C, D, F, G, O, H>
where 
    ...
    H: IsHBank + Party + 'static,
{
    ...
    pub fn add_contract_to_cohort(&mut self, cohort_id: String, mut contract: HealthDataContract<A, B, C, D, F, G, O, H>) -> Result<(), String> {
    ...
    pub fn get_contracts_by_cohort(&self, cohort_id: &str) -> Option<&Vec<HealthDataContract<A, B, C, D, F, G, O, H>>> {
        self.cohorts.get(cohort_id)
    }

    // Method to remove a cohort
    pub fn remove_cohort(&mut self, cohort_id: &str) -> Option<Vec<HealthDataContract<A, B, C, D, F, G, O, H>>> {
        self.cohorts.remove(cohort_id)
    }

    // Method to set cohort_id of a contract to None
    pub fn set_cohort_id_to_none(&mut self, contract: &mut HealthDataContract<A, B, C, D, F, G, O, H>) {
